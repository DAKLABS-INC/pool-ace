...
const mockPoolDetails = {
  1: {
    ...
    totalPool: 1200,
    description:
      "Premier League showdown between Manchester City and Arsenal. Winner takes all based on match result!",
  ...
  2: {
    ...
    totalPool: 600,
    description:
      "Epic NBA rivalry game between Lakers and Warriors. Private pool for serious basketball fans!",
...
const PoolDetails = () => {
  ...
  const pool =
    mockPoolDetails[parseInt(id || "0") as keyof typeof mockPoolDetails];
  ...
  const isParticipant =
    user && pool.participantsList.some((p) => p.id === user.id);
  const canJoin =
    user && !isParticipant && pool.participants < pool.maxParticipants;
                ...
                <div className="space-y-3">
                  {pool.participantsList.map((participant) => {
                    const contributionPercentage = (
                      (participant.betAmount / pool.totalPool) *
                      100
                    ).toFixed(1);
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <Badge
                            variant={getBetVariant(participant.betChoice)}
            ...
            {!user && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Sign in to participate in this pool
                  </CardDescription>
                </CardHeader>
                <CardContent>
            ...
            {canJoin && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Enter your bet amount to join this pool
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div>
                    <Label htmlFor="bet-side">Who will win?</Label>
                    <Select value={betSide} onValueChange={setBetSide}>
                      <SelectTrigger>
                        <SelectValue placeholder="Choose team or none" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="win">
                          {pool.title.split(":")[1]?.split("vs")[0]?.trim() ||
                            "Team 1"}
                        </SelectItem>
                        <SelectItem value="loss">
                          {pool.title.split("vs")[1]?.trim() || "Team 2"}
                        </SelectItem>
                        <SelectItem value="none">None</SelectItem>
                      </SelectContent>
                    </Select>
                    <div className="text-xs text-muted-foreground mt-2">
                      Select the team you believe will win, or choose{" "}
                      <b>None</b> if you do not want to bet on either team.
                    </div>
                  </div>
              ...
              !isParticipant && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      This pool has reached its maximum capacity
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
